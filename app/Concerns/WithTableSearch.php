...
trait WithTableSearch
{
            ...
            foreach ($fields as $field) {
                if (str_contains($field, '.')) {
                    ...
                    $q->orWhereHas($relation, function (Builder $relationQuery) use ($column, $term): void {
                        $this->applyLikeConstraint($relationQuery, $column, $term);
                    });
     ...
     * @param  Builder<TModel>  $query
     */
    protected function applyLikeConstraint(Builder $query, string $field, string $term): void
    {
        ...
        /** @phpstan-ignore argument.type (grammar->wrap() returns a safe identifier, not user input) */
